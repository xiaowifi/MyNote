...
对于H264的解码器而言，都要将SPS和PPS 信息传递给解码器。但是FFmepg解码视频的时候，并没有显示的设置SPS和PPS信息。因为在FFmpeg内部自己做了设置。但是对于硬件编码器而言，开发者必须手动设置，另外，使用FFmpeg解码出来的视频帧是以YUV格式的表示形式存储与内存中，但是对于硬件解码器而言，一般都是直接解码到显存中，便于后续的处理与渲染。

Android的算法都会在CPU上进行计算。

## 直播应用的构建

在直播领域，大致可以分为两种类型的直播，一种是非交互式直播，另外一种是交互式直播。非交互式直播的特点是源比较少、适合做多路转码。而交互直播中间服务器只作为一个中转的角色。直播中，传输介质是网络，而网络中传播视频或者音频需要使用对应的协议。目前适合做直播场景的协议有：

* rtmp 协议，长链接，低延时，网络穿透性差
* HLS 协议，Http的流媒体协议，高延时10s以上，跨平台性比较好。
* HDL 协议，RTMP协议的升级版本，低延时 2s左右，网络穿透性好
* RTP协议，低延时，1秒以内。默认使用UDP作为传输协议。

直播中的拉流播放器的使用时长，所占用的CPU资源会较多，建议使用硬件解码。后期也需要做一些处理，增加清晰度，还可以添加滤镜等。

Android 播放器增加后期处理过程流程图：

![image-20220810191104684](assets/image-20220810191104684.png)

直播应用的整体结构：

![image-20220810191300070](assets/image-20220810191300070.png)

从整体结构来看，录播和直播的区别仅在于最终的Muxer模块的输出不同。
...
网络环境问题，解决方案就是：实时根据主播所处的网络环境，调整主播的视频码率。如果网络出口带宽小于编码器产生的数据量，那就降低视频质量（降低编码器的码率设置，同时减小视频的FPS以免视频质量太差）以使主播可以流畅直播。由于这个流的码率中，视频轨码率达到90%以上（视频轨码率默认设置为600Kbps,音频轨码率默认设置为64kbps）,所以只需要处理视频轨的设置就可以满足大多数场景。

#### 推流数据统计

统计的数据包括但不限于开始直播时间、链接CDN推流节点服务器端连接时长、推流时长、丢帧率、推流的平均码率、自动降码率的变动表等。
...
![image-20220811090944083](assets/image-20220811090944083.png)
...
在Android平台上，显示部分使用的是SurfaceView,依托于其生命周期方法，将Surface 传递到Native 层，构造出ANativeWindow,然后构建成为EGLDisplay,最终由OpenGL ES 渲染上去。可以想办法脱离SurfaceVIew 生命周期控制。
...
在摄像头和麦克风越来越好，单纯依靠CPU的计算能力区处理音视频是远远不够的，在视频处理方面，多会使用openGL ES 技术，利用显卡的并行计算能力来提高处理图像或视频的速度。但是在音频处理方面却很少有比较成熟的技术提供开发者使用。ARM NEON技术采用SIMD(单指令，多数据)体系结构，可以有效提升多媒体与信号处理应用程序的性能。

Ne10是ARM主导开发的一个开源库。该库提供一系列通用的，基于ARM NEON架构并且经过深度优化的函数集合，通过调用这些库函数，可以让软件开发者免于编写重复的底层汇编代码，同时也能充分利用ARM NEON SIMD 指令的并行运算能力。
